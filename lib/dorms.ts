...
export async function getDormData(name: string) {
  try {
    ...
    const data: Dorm[] = await res.json();
    const filteredDorm = data.find((dorm: Dorm) => dorm.name.toLowerCase() === name.toLowerCase());

    if (!filteredDorm) throw new Error('Dorm not found');

    // Fetch reviews
    const reviewsResponse = await fetch(`${baseUrl}/api/reviews/${filteredDorm.id}`);
